...
import { Trophy, Clock, Zap, Sparkles, Star, AlertTriangle } from 'lucide-react';
...
// Visual Layout of the Wheel (12 Segments)
// This determines what the user SEES.
// We map the calculated result to one of these indices.
const WHEEL_SEGMENTS = [
  { value: 10, color: '#fbbf24', text: '#78350f', label: '10 CR' }, // 0: Jackpot (Gold)
  { value: 0, color: '#ef4444', text: '#ffffff', label: '0' },      // 1: Red
  { value: 2, color: '#3b82f6', text: '#ffffff', label: '2' },      // 2: Blue
  { value: 1, color: '#22c55e', text: '#ffffff', label: '1' },      // 3: Green
  { value: 3, color: '#a855f7', text: '#ffffff', label: '3' },      // 4: Purple
  { value: 0, color: '#ef4444', text: '#ffffff', label: '0' },      // 5: Red
  { value: 5, color: '#f97316', text: '#ffffff', label: '5' },      // 6: Orange
  { value: 1, color: '#22c55e', text: '#ffffff', label: '1' },      // 7: Green
  { value: 2, color: '#3b82f6', text: '#ffffff', label: '2' },      // 8: Blue
  { value: 0, color: '#ef4444', text: '#ffffff', label: '0' },      // 9: Red
  { value: 3, color: '#a855f7', text: '#ffffff', label: '3' },      // 10: Purple
  { value: 1, color: '#22c55e', text: '#ffffff', label: '1' },      // 11: Green
];

// Probability Weights (Total ~100,000 units)
// 0 Cr: 40%
// 1 Cr: 30%
// 2 Cr: 20%
// 3 Cr: 10%
...
const PRIZE_PROBABILITIES = [
    { value: 0, weight: 40000 },
    { value: 1, weight: 30000 },
    { value: 2, weight: 20000 },
    { value: 3, weight: 10000 },
    { value: 5, weight: 1000 },
    { value: 10, weight: 1 } 
];

const TOTAL_WEIGHT = PRIZE_PROBABILITIES.reduce((acc, curr) => acc + curr.weight, 0);
...
export const SpinWheel: React.FC<Props> = ({ user, onUpdateUser }) => {
  ...
  const handleSpin = () => {
    ...
    // Adding randomness within the segment so it doesn't always land dead center
    ...
    setRotation(prev => prev + finalRotation); // Add to previous rotation to spin forward
    ...
    setTimeout(() => {
      ...
      if (wonCredits > 0) {
        ...
        const updatedUser = { 
            ...user, 
            credits: user.credits + wonCredits, 
                ...
                return (
                    <div 
                        ...
                        style={{
                            ...
                            transformOrigin: '0% 100%', // Rotate around center
                        }}
                    >
                        <div 
                           ...
                           style={{ 
                               ...
                               transform: `skewY(${90 - SEGMENT_ANGLE}deg)`, // Unskew content container
                        ...
                        >
                            ...
                            <div 
                                ...
                                style={{ 
                                    color: seg.text,
                                    transform: `rotate(${SEGMENT_ANGLE/2}deg)`, // Rotate text to be upright relative to segment
                                    textShadow: '0px 1px 2px rgba(0,0,0,0.3)'
                                }}
                            >
                                <span className={seg.value >= 5 ? 'text-xl' : 'text-lg'}>{seg.label}</span>
                                {seg.value >= 5 && <Star size={12} className="inline ml-1 mb-1 fill-current" />}
                            </div>
                        </div>
                    </div>
                );
